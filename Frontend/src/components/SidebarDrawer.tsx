...
export const SidebarDrawer: React.FC<SidebarDrawerProps> = ({ isOpen, onClose, role }) => {
  ...
  const activeRole = (role || currentUser?.role || 'customer').toLowerCase()
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className={`fixed inset-0 z-50 bg-slate-900/50 transition-opacity duration-200 ease-out ${
      ...
      <aside
        className={`fixed top-0 left-0 bottom-0 z-50 w-80 max-w-[85vw] bg-white shadow-2xl transition-transform duration-200 ease-out transform-gpu will-change-transform flex flex-col ${
